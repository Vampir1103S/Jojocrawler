...
public class BetonZaun extends Environment {
    ...
    public BetonZaun(int x, int y) {
        this.x = x;
        this.y = y;
        double random = Math.random()*100;
        try {
            if (random >= 50) {
                image = ImageIO.read(new File("src/main/resources/graphic/BetonZaun.png"));
            }else{
                image = ImageIO.read(new File("src/main/resources/graphic/BetonZaun2.png"));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
